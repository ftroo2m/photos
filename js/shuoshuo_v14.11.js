  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (data[i].resources && data[i].resources.length > 0) {
      ...
      for (var j = 0; j < resources.length; j++) {
        ...
        if (restype == 'image') {
          imgUrl += '<figure class="gallery-thumbnail"><img class="img thumbnail-image" src="' + memos + 'file/' + resources[j].name+ '"/></figure>'
          resImgLength = resImgLength + 1
        }
        if (restype !== 'image') {
          resUrl += '<a target="_blank" rel="noreferrer" href="' + memos + 'file/' + resourceList[j].name + '">' + resourceList[j].name + '</a>'
        }
      }
      if (imgUrl) {
        var resImgGrid = ""
    ...
    result += '<li class="bb-list-li"><div class="bb-div"><div class="datatime"><div class="hy-avatar-block"><a href="' + bbMemo.userlink + '"class="hy-astyle"><img src="' + bbMemo.useravatar + '"class="hy-avatar"></a></div><div class="hy-intro"><div class="hy-name">' + bbMemo.username + '</div><div><span class="hy-time hy-text-muted">' + formatDate(data[i].createTime) + '</span></div></div></div><div class="datacont"><div>' + bbContREG + '</div></div><div class="hy-tag hy-text-muted"><span class="hy-location">' + (bbMemo.location == undefined ? "" : bbMemo.location) + '</span><span class="hy-tags-item">' + (bbMemo.tags == undefined ? "" : bbMemo.tags) + '</span><span><a data-id="' + data[i].name + '" data-site="' + artalkInit.site + '"  data-server="' + artalkInit.server + '" class="commentsLink" onclick="loadArtalk(this)">' + (bbMemo.commentsShow ? bbMemo.commentsTitle : "") + ' ' + '<span class="atk-comment-count-num" data-page-key="/m/' + data[i].uid + '"></span></a ></span></div><div id="' + data[i].name + '" class="comment d-none"></div></div></li>';
  }
